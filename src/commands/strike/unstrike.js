...
module.exports = class StrikeCommand extends Command {
  ...
  hasPermission(message) {
    ...
    if (message.guild) {
      return (message.member.roles.some(role => process.env.ALLOWED_ROLES.split(', ').includes(role.name)) || 
              message.member.permissions.has("ADMINISTRATOR")) ||
              message.member.id === process.env.OWNER_ID ? 
              true : 'The `unstrike` command requires you to have "Administrator" permission, or one of the following roles: ' + process.env.ALLOWED_ROLES.split(', ').map(role => `"${role}"`).join(', ') + '.';
    }
